...
class MetricsCollector : Emittery() {
    ...
    suspend fun _buildHandleHTTPDownloadRecordTask() {
        this._taskManager.createCyclicTask(
            MetricsCollectorTaskName.HANDLE_HTTP_DOWNLOAD_RECORD,
    ...
    suspend fun _buildHandleHTTPDownloadPulseTrafficTask() {
        this._taskManager.createCyclicTask(
            MetricsCollectorTaskName.HANDLE_HTTP_DOWNLOAD_PULSE_TRAFFIC,
    ...
    suspend fun _buildHandleHTTPDownloadCumulativeTrafficTask() {
        this._taskManager.createCyclicTask(
            MetricsCollectorTaskName.HANDLE_HTTP_DOWNLOAD_CUMULATIVE_TRAFFIC,
            HTTPDownloadCumulativeTrafficHandler(HTTPDownloadCumulativeTrafficHandlerOptions()),
            sleepFirst = true,
            sleepSeconds = 3.0,
            sleepJitter = 0.0
        )
    ...
    suspend fun _buildHandleHTTPDownloadWMABandwidthTask() {
        this._taskManager.createCyclicTask(
            MetricsCollectorTaskName.HANDLE_HTTP_DOWNLOAD_WMA_BANDWIDTH,
            HTTPDownloadWMABandwidthHandler(HTTPDownloadWMABandwidthHandlerOptions()),
            sleepFirst = true,
            sleepSeconds = 3.0,
            sleepJitter = 0.0
        )
    ...
    suspend fun _buildHandleHTTPDownloadUsagePulseCountTask() {
        this._taskManager.createCyclicTask(
            MetricsCollectorTaskName.HANDLE_HTTP_DOWNLOAD_USAGE_PULSE_COUNT,
            HTTPDownloadUsagePulseCountHandler(
                HTTPDownloadUsagePulseCountHandlerOptions()
            ),
            sleepFirst = true,
            sleepSeconds = 3.0,
            sleepJitter = 0.0
    ...
    suspend fun _buildHandleHTTPDownloadUsageCumulativeCountTask() {
        this._taskManager.createCyclicTask(
            MetricsCollectorTaskName.HANDLE_HTTP_DOWNLOAD_USAGE_CUMULATIVE_COUNT,
            HTTPDownloadUsageCumulativeCountHandler(
                HTTPDownloadUsageCumulativeCountHandlerOptions()
            ),
            sleepFirst = true,
            sleepSeconds = 3.0,
            sleepJitter = 0.0
    ...
    suspend fun _buildHandleHTTPDownloadSuccessPulseCountTask() {
        this._taskManager.createCyclicTask(
            MetricsCollectorTaskName.HANDLE_HTTP_DOWNLOAD_SUCCESS_PULSE_COUNT,

            HTTPDownloadSuccessPulseCountHandler(
                HTTPDownloadSuccessPulseCountHandlerOptions()
            ), sleepFirst = true, sleepSeconds = 3.0, sleepJitter = 0.0
        )
    }

    suspend fun _buildHandleHTTPDownloadSuccessCumulativeCountTask() {
        this._taskManager.createCyclicTask(
            MetricsCollectorTaskName.HANDLE_HTTP_DOWNLOAD_SUCCESS_CUMULATIVE_COUNT,

            HTTPDownloadSuccessCumulativeCountHandler(CursorsTimeSeriesDataData()),
            sleepFirst = true,
            sleepSeconds = 3.0,
            sleepJitter = 0.0
        )
    ...
    suspend fun _buildHandleHTTPDownloadFailurePulseCountTask() {
        this._taskManager.createCyclicTask(
            MetricsCollectorTaskName.HANDLE_HTTP_DOWNLOAD_FAILURE_PULSE_COUNT,

            HTTPDownloadFailurePulseCountHandler(CursorsTimeSeriesDataData()),
            sleepFirst = true,
            sleepSeconds = 3.0,
            sleepJitter = 0.0
        )
    ...
    suspend fun _buildHandleHTTPDownloadFailureCumulativeCountTask() {
        this._taskManager.createCyclicTask(
            MetricsCollectorTaskName.HANDLE_HTTP_DOWNLOAD_FAILURE_CUMULATIVE_COUNT,

            HTTPDownloadFailureCumulativeCountHandler(CursorsTimeSeriesDataData()),
            sleepFirst = true,
            sleepSeconds = 3.0,
            sleepJitter = 0.0
        )
    ...
    suspend fun _buildHandleCDNConfigRecordsTask() {
        this._taskManager.createCyclicTask(
            MetricsCollectorTaskName.HANDLE_CDN_CONFIG_RECORDS,
            CDNConfigRecordsHandler(),
            sleepSeconds = 0.0
        )
    }
    ...
    suspend fun _buildHandleCDNDownloadRecordTask() {
        this._taskManager.createCyclicTask(
            MetricsCollectorTaskName.HANDLE_CDN_DOWNLOAD_RECORD,
            CDNDownloadRecordHandler(),
            sleepSeconds = 0.0
        )
    }
    ...
    suspend fun _buildHandleCDNDownloadLastMeanBandwidthTask() {
        this._taskManager.createCyclicTask(
            MetricsCollectorTaskName.HANDLE_CDN_DOWNLOAD_LAST_MEAN_BANDWIDTH,

            CDNDownloadLastMeanBandwidthHandler(CursorsTimeSeriesDataData()),
            sleepFirst = true,
            sleepSeconds = 3.0,
            sleepJitter = 0.0
        )
    ...
    suspend fun _buildHandleCDNDownloadLastMeanAvailabilityTask() {
        this._taskManager.createCyclicTask(
            MetricsCollectorTaskName.HANDLE_CDN_DOWNLOAD_LAST_MEAN_AVAILABILITY,

            CDNDownloadLastMeanAvailabilityHandler(CursorsTimeSeriesDataData()),
            sleepFirst = true,
            sleepSeconds = 3.0,
            sleepJitter = 0.0
        )
    ...
    suspend fun _buildHandleCDNDownloadLastCurrentScoreTask() {
        this._taskManager.createCyclicTask(
            MetricsCollectorTaskName.HANDLE_CDN_DOWNLOAD_LAST_CURRENT_SCORE,

            CDNDownloadLastCurrentScoreHandler(
                CDNDownloadLastMeanAvailabilityHandlerOptions()
            ), sleepFirst = true, sleepSeconds = 3.0, sleepJitter = 0.0
        )
    }
