...
data class CursorsTimeSeriesDataData(override var cursors: MutableMap<String, TimeSeriesDataData<Double>> = mutableMapOf()) :
...
class CDNConfigRecordsHandler() : AbstractFlow<Unit>(content = Unit) {
    override suspend fun process() {
        this._intakeRecords()
...
class PruneMetricsStatsHandler(content: PurgeCDNRecordsHandlerOptions) :
    ...
    suspend fun _updateSource() {
        val metrics = MetricsStatsHolder.source ?: return
